      ...
      <section className="py-6 bg-white">
        {collections.map((c, idx) => (
          <div key={c.id} className={`container-x py-16 grid md:grid-cols-2 gap-10 items-center ${idx % 2 ? 'md:[&>*:first-child]:order-2' : ''}`}>
            ...
            >
              <img src={c.image} alt={`${c.name} collection — ${c.tagline}`} className="w-full h-full object-cover" loading="lazy" decoding="async" />
            </motion.div>
            ...
            >
              ...
              <Button to="/shop" variant="primary">Shop {c.name}</Button>
            </motion.div>
          </div>
        ))}
      </section>
